...
import { Bus, MapPin, Clock, CreditCard, Wallet, Smartphone, CheckCircle, ArrowLeft, Minus, Plus } from "lucide-react";
...
const BookingPage = () => {
  ...
  // Mock bus data (would come from API based on searchParams.get("bus"))
  const bus = {
    id: 1,
    number: "42A",
  ...
  const [toStop, setToStop] = useState(stops[stops.length - 1]);

  const calculateFare = () => {
    const fromIndex = stops.indexOf(fromStop);
    const toIndex = stops.indexOf(toStop);
    const distance = Math.abs(toIndex - fromIndex);
    const baseFare = 0.50; // Base fare
    const farePerStop = 0.40; // Cost per stop
              ...
              {step === 2 && (
                ...
                >
                  ...
                  <Button size="lg" className="w-full" onClick={handleBooking} disabled={isProcessing}>
                    {isProcessing ? "Processing..." : `Pay $${totalFare.toFixed(2)}`}
                  </Button>
                </motion.div>
              )}
                ...
                >
                  ...
                  <div className="bg-gradient-to-br from-primary to-accent rounded-2xl p-6 text-primary-foreground text-left max-w-sm mx-auto">
                    ...
                    <div className="grid grid-cols-3 gap-4 pt-4 border-t border-primary-foreground/20">
                      <div>
                        <p className="text-primary-foreground/70 text-xs">Time</p>
